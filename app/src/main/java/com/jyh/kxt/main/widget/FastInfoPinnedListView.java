 ...
 */
public class FastInfoPinnedListView extends ListView {
    ...
    private final OnScrollListener mOnScrollListener = new OnScrollListener() {
        ...
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            ...
            if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
                if (view.getLastVisiblePosition() == view.getCount() - 1) {    // 判断是否滚动到底部
                    if (footerListener != null && currentPullStatus == 0) {
                        showFoot();
                    }
                }
            }
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            if (totalItemCount > 20 && (firstVisibleItem + visibleItemCount) == totalItemCount - 10) {
                showFoot();
            }
        }
    };
    ...
    public void initFootTvView() {
        footView = new TextView(getContext());
        footView.setTextColor(0xffA5A5A5);
        footView.setText("加载中...");
        float dimension = getResources().getDimension(R.dimen.month_list_item_header_height);
    ...
    private void showFoot() {
        ...
        if (currentPullStatus == 0 && !footView.isShown()) {
            currentPullStatus = 1;
            footView.setVisibility(View.VISIBLE);
            footerListener.startLoadMore();
            timeOutHandler.sendMessageDelayed(timeOutHandler.obtainMessage(), 10 * 1000);
        }
    }
    ...
    public FastInfoPinnedListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public FastInfoPinnedListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    void createPinnedShadow(int position) {
        ...
        View pinnedView = getAdapter().getView(position, pinnedShadow.view, FastInfoPinnedListView.this);
    ...
    private Set<View> headerView = new HashSet<>();
    ...
    public void removeHeaderView() {
        Iterator<View> views = headerView.iterator();
        while (views.hasNext()) {
            View oldHeadView = views.next();
            if (oldHeadView instanceof ViewGroup) {
                ViewGroup oldHeadViewGroup = (ViewGroup) oldHeadView;
                oldHeadViewGroup.removeAllViews();
            }
            removeHeaderView(oldHeadView);
        }
    }
    ...
    public void invalidatePinnedView() {
        try {
            if (mPinnedSection != null && mPinnedSection.view != null) {
                ...
                int color1 = ContextCompat.getColor(getContext(), R.color.line_color2);
                mPinnedSection.view.setBackgroundColor(color1);

                int color2 = ContextCompat.getColor(getContext(), R.color.font_color3);
                ((TextView) mPinnedSection.view.findViewById(R.id.tv_time_day)).setTextColor(color2);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
