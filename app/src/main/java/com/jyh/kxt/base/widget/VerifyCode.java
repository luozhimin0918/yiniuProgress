...
//        typedArray.recycle();
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.VerifyCode);
        mCodeTextSize = array.getDimensionPixelSize(R.styleable.VerifyCode_codeTextSize, 16);
        mCodeBackground = array.getColor(R.styleable.VerifyCode_codeBackground, ContextCompat.getColor(context, R.color.theme1));
        mCodeLength = array.getInt(R.styleable.VerifyCode_codeLength, 4);
        isContainChar = array.getBoolean(R.styleable.VerifyCode_isContainChar, false);
        mPointNum = array.getInt(R.styleable.VerifyCode_pointNum, 100);
        mLineNum = array.getInt(R.styleable.VerifyCode_linNum, 3);
        array.recycle();
    }
...
// UNSPECIFIED：表示子布局想要多大就多大，很少使用
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            mPaint.setTextSize(SystemUtil.dp2px(getContext(), mCodeTextSize));
            mPaint.getTextBounds(mCodeText, 0, mCodeText.length(), mBound);
    ...
    private Bitmap createBitmapValidate() {
        ...
        for (int i = 1; i <= length; i++) {
            ...
            //设置字体的绘制位置
            canvas.drawText(String.valueOf(mCodeText.charAt(i - 1)), (mWidth - textWidth) / 2 + (i - 1) * charLength + 5,
                    mHeight * 4 / 5f, mPaint);
